    ...
    this.writePromise = (async () => {
      ...
      if (options?.onConflict) {
        ...
        if (conflictValue !== undefined) {
          ...
          const q = query(colRef, where(conflictField, "==", conflictValue));
...
class FirebaseChannel {
  ...
  private eventCallback?: (payload: any) => void;
  ...
  on(eventType: string, config: any, callback: (payload: any) => void) {
    ...
    if (match) {
      this.filterField = match[1];
      this.filterValue = match[2];
    }
  ...
  subscribe(statusCallback?: (status: string) => void) {
    if (!this.tableName || !this.eventCallback) {
      if (statusCallback) statusCallback("SUBSCRIBED");
      return this;
    }

    const mappedCollection = COLLECTION_MAPPING[this.tableName] || this.tableName;

    let isFirst = true;
    const colRef = collection(db, mappedCollection);
    let q = query(colRef);
    if (this.filterField && this.filterValue) {
      q = query(q, where(this.filterField, "==", this.filterValue));
    }
      ...
      (snapshot) => {
        snapshot.docChanges().forEach((change) => {
          if (change.type === "added") {
            ...
            const mappedData: any = { id: change.doc.id, ...data };
            if (mappedData.churchId !== undefined && mappedData.church_id === undefined) {
              mappedData.church_id = mappedData.churchId;
            }
            if (mappedData.createdAt !== undefined && mappedData.created_at === undefined) {
              mappedData.created_at = mappedData.createdAt;
            }
            if (mappedData.updatedAt !== undefined && mappedData.updated_at === undefined) {
              mappedData.updated_at = mappedData.updatedAt;
            }

            this.eventCallback!({ new: mappedData });
          }
        });
        isFirst = false;
      },
...
export const supabase = {
  from: (tableName: string) => new ClientQueryBuilder(tableName),
  channel: (channelName: string) => new FirebaseChannel(channelName),
